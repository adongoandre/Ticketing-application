...
<body id="page-top" class="bg-dark">
    ...
    <main>
        <div class="container-fluid mt-4 p-3 pb-0">
            <div class="d-flex align-items-center justify-content-between ">
                <h1 class="h3 mb-0 text-white-700">Tickets</h1>
                <div>
                    <button type="button" class="btn btn-sm b-color p-2 shadow-sm" data-bs-toggle="modal" data-bs-target="#add-ticket">
                        + Add Ticket
                    </button>
                </div>
            </div>
                ...
                <tbody class="border-dark" id="userTableBody">
				    <c:forEach var="ticket" items="${tickets}">
				        <tr>
				            ...
				            <td>
				            	&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="#" class="btn b-color" data-bs-toggle="modal" data-bs-target="#edit-ticket"
								   data-num="${ticket.num}" 
								   data-description="${ticket.description_}" 
								   data-priority="${ticket.priority}"
								   data-image-url="${pageContext.request.contextPath}/ticket-image?num=${ticket.num}">
								   Edit
								</a>
				                &nbsp;&nbsp;&nbsp;&nbsp;
				                <a href="ticket?action=delete&id=<c:out value='${ticket.num}'/>" class="btn btn-dark" onclick="return confirm('Are you sure you want to delete this user?');">Delete</a>
				            </td>
				        </tr>
				    </c:forEach>
				</tbody>
    ...
    <!--Add ticket modal-->
    <div class="modal fade" id="add-ticket" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-dark text-light">
                <div class="modal-header d-flex flex-sm-column align-items-center justify-content-center">
                    <label class="form-label h3">Add Ticket</label>
                </div>
                <div class="modal-body">
                    <form action="ticket?action=insert" method="post" enctype="multipart/form-data" class="py-3" id="ticketForm">
    ...
    <!-- edit modal -->
    <div class="modal fade" id="edit-ticket" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	    <div class="modal-dialog modal-dialog-centered">
	        <div class="modal-content bg-dark text-light">
	            <div class="modal-header d-flex flex-sm-column align-items-center justify-content-center">
	                <label class="form-label h3">Edit Ticket</label>
	            </div>
	            <div class="modal-body">
	                <form action="ticket?action=update" method="post" enctype="multipart/form-data" class="py-3" id="ticketForm">
	                    <input type="hidden" name="num" id="ticketId" value="${ticket.num}">					    
	                    <div class="mt-2 d-flex flex-column align-items-center justify-content-center">
	                        <textarea class="form-control length text-black" name="description" id="ticket">${ticket.description_}</textarea>
	                    </div>					
	                    <div class="mt-4 d-flex flex-column align-items-center justify-content-center">
	                        <div class="form-check">
	                            <label>
	                    ...
	                    <div class="mt-4 d-flex flex-column align-items-center justify-content-center">
						    <h6>Do you want to change this image?</h6>
						    <img id="editTicketImage" src="${pageContext.request.contextPath}/ticket-image?num=${ticket.num}" 
						         alt="No ticket image" 
						         class="thumb img-thumbnail" 
						         style="width: auto; height: 250px; cursor: pointer;"/>
						    <input type="file" name="image" class="form-control-file" id="image">
						    <!-- Hidden input field to track if the user wants to keep the existing image -->
